...
if __name__ == "__main__":
    ...
    # Left Join
    joined = trip_values.leftOuterJoin(demog_values)
    joined_csv = joined.map(lambda res: str(res[1][0]) + ',' + str(res[0]) + ',' + str(res[1][1]))
    output = joined_csv.saveAsTextFile(sys.argv[3])
    sc.stop()


spark-submit join_dataset.py trip_test ../../data/demographics.csv
